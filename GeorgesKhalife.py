...
# Game --> Subclass of Tkinter Frame
class Game(Frame):
    ...
    def initUI(self):
        ...
        prompt = Label(self.title_frame, text="Click the cells to create your starting config, then press Start Game:")
        ...
        # creates a glider
        self.glider_button = Button(self.parent, text="Glider", command= lambda: self.glider())
        self.glider_button.grid(row=1, column=3, sticky=W)

        # randomizes board
        self.random_button = Button(self.parent, text="Randomize", command = lambda: self.randomize())
        self.random_button.grid(row=1, column=0, sticky=W)

        # creates a blinker
        self.blinker_button = Button(self.parent, text="Blinker", command = lambda: self.blinker())
        self.blinker_button.grid(row=0, column=3, sticky=W)

        # creates a toad
        self.toad_button = Button(self.parent, text="Toad", command = lambda: self.toad())
    ...
    def create_grid(self):
        ...
        self.game_frame = Frame(self.parent, width=self.max_x + 2, height=self.max_y + 2, borderwidth=1, relief=SUNKEN)
        self.game_frame.grid(row=2, column=0, columnspan=4)

        # instantiates buttons for choosing initial configuration
        self.cell_buttons = [[Button(self.game_frame, bg="white", width=2, height=1) for i in range(self.max_x + 2)] for j in range(self.max_y + 2)]
